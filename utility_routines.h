...
**
**	24-NOV-2019	RRL	Fixed bug in the __util#sub_time()
*/
...
inline static void __util$sub_time(struct timespec * time1, struct timespec* time2, struct timespec* result_time)
{
	if ( (time1->tv_nsec - time2->tv_nsec) < 0 )
	        {
		result_time->tv_sec = time1->tv_sec - time2->tv_sec - 1;
		result_time->tv_nsec = time1->tv_nsec - time2->tv_nsec + 1000000000;
	        }
	else	{
		result_time->tv_sec = time1->tv_sec - time2->tv_sec;
		result_time->tv_nsec = time1->tv_nsec - time2->tv_nsec;
	        }
}
